...
def show_deployment_info(api_key: str):
    ...
    print("  • Apply .env changes: docker compose up -d")
    print(f"  • Update: python {sys.argv[0]} deploy")
    print()

    print(f"{Colors.CYAN}🤖 Connecting Claude:{Colors.NC}")
    print("  Claude connects to remote MCP servers as a custom connector, not through")
    print("  claude_desktop_config.json. Put the server behind HTTPS, then add")
    print("  https://<your-host>/mcp under Settings > Connectors > Add custom connector.")
    print("  See docs/CLAUDE_INTEGRATION.md.")
    print(f"  Local MCP clients that send headers: http://localhost:{port}/mcp")
    print("  with Authorization: Bearer <token from POST /auth/token>.")
    print()
