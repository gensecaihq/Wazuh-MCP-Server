...
async def require_authorization(authorization: Optional[str]) -> str:
    """
    Validate Authorization header for both JWT (from /auth/token) and legacy in-memory tokens.
    Returns the auth type used ("jwt" or "session"); raises HTTPException on failure.
    """
    ...
    # First, try JWT generated by /auth/token (HS256 signed with AUTH_SECRET_KEY)
    try:
        verify_token(raw_token, config.AUTH_SECRET_KEY)
        return "jwt"
    except ValueError as jwt_error:
        jwt_error_msg = str(jwt_error)

    # Fallback to in-memory bearer tokens (wst_*) used by legacy clients
    try:
        from wazuh_mcp_server.auth import verify_bearer_token
        await verify_bearer_token(authorization)
        return "session"
    except ValueError as bearer_error:
        raise HTTPException(
            status_code=401,
            detail=str(bearer_error) or jwt_error_msg,
            headers={"WWW-Authenticate": "Bearer"}
        )
...
async def mcp_sse_endpoint(
    ...
    """
    # Authentication required for remote MCP servers (JWT or legacy session token)
    await require_authorization(authorization)
...
async def close_mcp_session(
    ...
    """
    # Authentication required (JWT or legacy session token)
    await require_authorization(authorization)
