...
- **[@markeclaudio](https://github.com/markeclaudio)** — OpenID Connect sign-in at an external identity provider (#123), the protected-target check on every IP-blocking tool (#124), guard-rails on destructive tools: confirmation, fleet-wide and Manager targets, quarantine paths (#125), session-store bounds (#126), and Manager TLS verification by default with `WAZUH_CA_BUNDLE` (#127).
